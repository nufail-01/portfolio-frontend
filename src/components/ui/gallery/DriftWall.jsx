...
//     // CHANGED: was "translate(-50%, -50%)" which re-centers the plane
//     // horizontally every single animation frame, silently undoing the
//     // CSS left-alignment fix in DriftWall.css. Only vertical centering
//     // should happen here.
...
//           const factor =
//             paused || hoveredColRef.current === columnIndex ? 0 : 1;

//           const target = baseVelocities[columnIndex] * factor;

//           const easing =
//             1 - Math.exp(-deltaTime / (target === 0 ? 0.16 : 0.28));

//           velocitiesRef.current[columnIndex] +=
//             (target - velocitiesRef.current[columnIndex]) * easing;

//           let next =
//             (offsetsRef.current[columnIndex] ?? 0) +
//             velocitiesRef.current[columnIndex] * deltaTime;
...
//     };
//   }, [applyPlaneTransform, baseVelocities, columnMeta, pauseOnHover, reduced]);
...
//       ...style,
//     }),
//     [tileWidth, tileHeight, gap, radius, lift, dim, grayscale, overlayColor, fade, style],
//   );
...
//     return (
//       <div key={id} tabIndex={0} role="button" aria-label={item.title ?? "Certificate"} {...commonProps}>
//         {inner}
//       </div>
//     );
//   };
...
const DriftWall = ({
   ...
   * IMPORTANT:
   *
   * The wall is now completely flat.
   *
   * No rotateX
   * No rotateY
   * No rotateZ
   ...
   * No translateZ
   *
   * We only center the plane horizontally
   * and vertically.
   * ------------------------------------------------------------
   */
